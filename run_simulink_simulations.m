...
e = makeExp( ...
    '02a_LQG_H2equiv_NL', ...
    'LQG - verifica equivalenza H2 in regolazione', ...
    'LQG', ...
    2, ...
    2, ...
    1, ...
...
e.q0Override = q0_H2equiv;
EXP(end+1) = e;


e = makeExp( ...
    '02b_H2_H2equiv_NL', ...
    'H2 classico - verifica equivalenza LQG in regolazione', ...
    'LQG', ...
    2, ...
    3, ...
    1, ...
...
e = makeExp( ...
    '01b_LQG_sensor_noise_NL', ...
    'LQG - plant NL con rumore di misura', ...
    'LQG',2,2,1,refCombined,aeroOff,deltaNominal);
e.sensorNoise   = 1;
e.actuatorNoise = 0;
EXP(end+1) = e;

% 2) Rumore di misura + rumore di attuazione
e = makeExp( ...
    '01c_LQG_sensor_actuator_noise_NL', ...
    'LQG - plant NL con rumore di misura e attuazione', ...
    'LQG',2,2,1,refCombined,aeroOff,deltaNominal);
e.sensorNoise   = 1;
e.actuatorNoise = 1;
EXP(end+1) = e;

% 3) Setup completo: rumori + disturbi aerodinamici
e = makeExp( ...
    '01d_LQG_full_realism_NL', ...
    'LQG - plant NL con rumori e disturbi aerodinamici', ...
...
% 4) LQGI nel setup completo: rumori + disturbi aerodinamici
e = makeExp( ...
    '04b_LQGI_full_realism_NL', ...
    'LQGI - plant NL con rumori e disturbi aerodinamici', ...
    'LQG',2,1,1,refCombined,aeroStep,deltaNominal);
e.sensorNoise   = 1;
e.actuatorNoise = 1;
EXP(end+1) = e;
...
EXP(end+1) = makeExp( ...
    '04c_H2soft_disturbance_NL', ...
    'H2 soft-integrator - reiezione disturbo costante', ...
    'LQG', ...
    2, ...
    4, ...
    1, ...
...
EXP(end+1) = makeExp( ...
    '04a_LQG_disturbance_eq_NL', ...
    'LQG - reiezione disturbo a riferimento costante', ...
    'LQG', ...
    2, ...
    2, ...
    1, ...
...
EXP(end+1) = makeExp( ...
    '04b_LQGI_disturbance_eq_NL', ...
    'LQGI - reiezione disturbo a riferimento costante', ...
    'LQG', ...
    2, ...
    1, ...
    1, ...
...
% ---- H2 soft-integrator: setup completo ---------------------------------

e = makeExp( ...
    '04d_H2soft_full_realism_NL', ...
    'H2 soft-integrator - plant NL con rumori e disturbi aerodinamici', ...
    'LQG', ...
    2, ...
    4, ...
    1, ...
...
e = makeExp( ...
    '04e_LQG_full_realism_eq_NL', ...
    'LQG - rumori e disturbi a riferimento costante', ...
    'LQG',2,2,1, ...
    refEquilibrium,aeroStep,deltaNominal);
...
e = makeExp( ...
    '04f_LQGI_full_realism_eq_NL', ...
    'LQGI - rumori e disturbi a riferimento costante', ...
    'LQG',2,1,1, ...
    refEquilibrium,aeroStep,deltaNominal);
...
compareRuns( ...
    ...
    comparisonDir, ...
    cfg, ...
    '12_LQG_vs_H2_regulation', ...
    'Verifica dell''equivalenza LQG-H2 in regolazione');
...
compareControls( ...
    ...
    comparisonDir, ...
    cfg, ...
    '12b_LQG_vs_H2_regulation_control', ...
    'Comando di controllo nella verifica LQG-H2');
...
compareRuns(outputs,EXP, ...
    ...
    {'Nominale', ...
     'Rumore misura', ...
     'Rumore misura + attuazione', ...
     'Rumori + disturbi aerodinamici'}, ...
    'LQG',SIG,comparisonDir,cfg, ...
    '10_LQG_progressive_validation_tracking', ...
    'Validazione progressiva del controllore LQG');

% 8.2b Validazione progressiva LQG: errore di tracking
compareErrors(outputs,EXP, ...
    {'01_LQG_nominal_NL', ...
     '01b_LQG_sensor_noise_NL', ...
     '01c_LQG_sensor_actuator_noise_NL', ...
    ...
    {'Nominale', ...
     ...
     'Rumori + disturbi aerodinamici'}, ...
    'LQG',SIG,comparisonDir,cfg, ...
    '11_LQG_progressive_validation_error', ...
    'Errore di tracking nella validazione progressiva LQG');

% 8.2c LQG vs LQGI nel setup completo: errore
compareErrors(outputs,EXP, ...
    {'01d_LQG_full_realism_NL','04b_LQGI_full_realism_NL'}, ...
    {'LQG','LQGI'},'LQG',SIG,comparisonDir,cfg, ...
    '12_LQG_vs_LQGI_full_realism_error', ...
    'LQG e LQGI con rumori e disturbi aerodinamici');
...
compareErrors( ...
    ...
    comparisonDir, ...
    cfg, ...
    '13_LQG_LQGI_H2soft_disturbance_error', ...
    'Reiezione del disturbo costante: LQG, LQGI e H2 soft');
...
compareControls( ...
    ...
    comparisonDir, ...
    cfg, ...
    '13b_LQG_LQGI_H2soft_disturbance_control', ...
    'Sforzo di controllo nella reiezione del disturbo');

% 8.2d LQG vs LQGI vs H2-soft nel setup completo: errore
...
compareErrors( ...
    ...
    {'LQG','LQGI','H2 soft'}, ...
    'LQG',SIG,comparisonDir,cfg, ...
    '14_LQG_LQGI_H2soft_full_realism_error', ...
    'LQG, LQGI e H2 soft con rumori e disturbi');

% 8.2e LQG vs LQGI vs H2-soft nel setup completo: controllo
...
compareControls( ...
    ...
    {'LQG','LQGI','H2 soft'}, ...
    'LQG',SIG,comparisonDir,cfg, ...
    '15_LQG_LQGI_H2soft_full_realism_control', ...
    'Sforzo di controllo con rumori e disturbi');

% 8.2d LQG vs LQGI nel setup completo: sforzo di controllo
compareControls(outputs,EXP, ...
    {'01d_LQG_full_realism_NL','04b_LQGI_full_realism_NL'}, ...
    {'LQG','LQGI'},'LQG',SIG,comparisonDir,cfg, ...
    '13_LQG_vs_LQGI_full_realism_control_effort', ...
    'Sforzo di controllo LQG e LQGI nel setup completo');
...
% 8.4 confronto nominale dei controllori H-infinity / PID strutturato
compareRuns(outputs,EXP, ...
    {'07_mixsyn_nominal_NL','08_hinfsyn_nominal_NL', ...
    '09_PIDcomp_nominal_NL'}, ...
    {'mixsyn','hinfsyn','PID + compensatore'}, ...
    'ROB',SIG,comparisonDir,cfg, ...
    '04_robust_controllers_tracking', ...
    'Confronto nominale dei controllori robusti sul modello non lineare');
...
compareControls(outputs,EXP, ...
    ...
    {'mixsyn','hinfsyn','PID + compensatore'}, ...
    'ROB',SIG,comparisonDir,cfg, ...
    '05_robust_controllers_control_effort', ...
    'Confronto dello sforzo di controllo nominale');

% 8.5 stress test comune: mixsyn vs mu-synthesis
compareRuns(outputs,EXP, ...
    {'11_mixsyn_uncertain_NL','12_mu_uncertain_NL'}, ...
    {'mixsyn','mu-synthesis'}, ...
    'ROB',SIG,comparisonDir,cfg, ...
...
%VALIDATEBASEWORKSPACEFORCAMPAIGN Verifica le variabili essenziali.
    ...
    required = { ...
        ...
        'K_LQG','K_LQGI','K_H2','K_H2_soft', ...
        'K_mix','K_hinfsyn','K_pidcomp','K_mu'};
...
function cleanupPreviousCampaign(outDir)
    ...
    % Elimina cartelle obsolete associate al vecchio H2 mixed-sensitivity.
    obsoleteDirs = { ...
        '10_H2_nominal_LIN', ...
        '10_H2_nominal_NL', ...
        '16_RAS_inside_H2', ...
        '17_RAS_outside_H2', ...
        'V3_H2_uncertain_matched_LIN', ...
        'V3_H2_uncertain_matched_NL' ...
        };

    for k = 1:numel(obsoleteDirs)

        d = fullfile(outDir,obsoleteDirs{k});
    ...
    % Elimina inoltre le vecchie directory comparative H2, se presenti.
    obsoleteComparisonDirs = { ...
        fullfile('linear_vs_nonlinear_closed_loop','08_H2_nominal'), ...
        fullfile('linear_vs_nonlinear_closed_loop','11_H2_uncertain_matched') ...
        };

    for k = 1:numel(obsoleteComparisonDirs)

        d = fullfile(outDir,obsoleteComparisonDirs{k});
