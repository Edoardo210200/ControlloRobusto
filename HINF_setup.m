...
for k = 1:numel(parametersToNominal)
    parName = ...
        parametersToNominal{k};
    if isfield( ...
            Pq_uncertain.Uncertainty, ...
            parName)
        block = ...
            Pq_uncertain.Uncertainty.(parName);
        Pq_uncertain = ...
            usubs( ...
                Pq_uncertain, ...
                parName, ...
...
G_uncertain_full = ...
    Pq_uncertain_full * ...
    Gact_uncertain;
G_uncertain_full.InputName = {
    'delta_F1_cmd'
    'delta_F2_cmd'
};
G_uncertain_full.OutputName = {
    'delta_alpha'
    'delta_beta'
};
...
weakOutputDirection = ...
    Udc(:,end);
figure('Name','Nominal plant singular values');
sigma(G_nominal);
grid on;
title('Valori singolari del plant nominale $G(j\omega)$','Interpreter','latex');
...
sigma( ...
    ...
    omegaWeights);
grid on;
title('$W_S^{-1}(j\omega)$ - limite sulla sensibilita''','Interpreter','latex');
subplot(3,1,2);
sigma( ...
    inv(WU), ...
    omegaWeights);
grid on;
title('$W_U^{-1}(j\omega)$ - limite sullo sforzo di controllo','Interpreter','latex');
subplot(3,1,3);
sigma( ...
    inv(WT), ...
    omegaWeights);
grid on;
title('$W_T^{-1}(j\omega)$ - limite sulla sensibilita'' complementare','Interpreter','latex');
